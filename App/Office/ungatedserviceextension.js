...
OSF.OmexProvider = (function () {
    var _hasBeenInitialized = false;
    var _clientID = '817DA9BD-44C3-4D7F-8BA9-4C3195E1269A';
    var _cacheKeyPrefix = '__OSF_OMEX.';
    var _appDetailKey = _cacheKeyPrefix + 'appDetails.{0}';
    var _recommendationsKey = _cacheKeyPrefix + 'recommendations.{0}.{1}.{2}.{3}';
    var _cacheManager = new OfficeExt.AppsDataCacheManager(OSF.OUtil.getLocalStorage(), new OfficeExt.SafeSerializer());
    var _urls = {
        recommendationWS: "/app/query",
        appDetailsWS: "/app/query"
    };
    var _queries = {
        recommendationWS: "?type=2&pmo={0}&cmo={1}&appt=0&av={2}&appst={3}",
        appDetailsWS: "?type=5&cmo={0}&rt=xml"
    };
    ...
    ;
    function _onGetAppDetailsCompleted(executor) {
        var context = executor.get_webRequest().get_userContext();
        ...
        for (var i = 0; i < waInfoNodes.length; ++i) {
            ...
            appDetail = {};
            xmlProcessor.readAttributes(waInfoNode, { "assetid": "assetId", "defhei": "defaultHeight", "defwid": "defaultWidth", "title": "name", "prov": "provider", "desc": "description", "icon": "iconUrl", "ver": "version", "state": "state", "appst": "appSubType", "appvers": "appVersions", "req": "requirements", "hosts": "hosts", "highresicon": "highResolutionIconUrl" }, appDetail);
            context.appDetails.push(appDetail);
        }
        _invokeCallbackTag(context.callback, statusCode.Succeeded, context.appDetails, null, executor, 0x0080b75b);
    }
    ;
    function _onGetRecommendationsCompleted(executor) {
        var context = executor.get_webRequest().get_userContext();
        var responseXml = executor.get_responseData();
        var xmlProcessor = new OSF.XmlProcessor(responseXml, _omexXmlNamespaces);
        var resultsNode = xmlProcessor.selectSingleNode("o:results");
        ...
        for (var i = 0; i < waNodes.length; ++i) {
            waNode = waNodes[i];
            recommendation = {};
            xmlProcessor.readAttributes(waNode, { "assetid": "assetId", "title": "name", "desc": "description", "icon": "iconUrl", "ver": "version", "state": "state", "rat": "rating", "dls": "downloads", "prov": "provider" }, recommendation);
            priceNode = xmlProcessor.selectSingleNode("o:price", waNode);
            xmlProcessor.readAttributes(priceNode, { "fop": "originalPrice", "opt": "originalPriceTier", "fpp": "promotionalPrice", "ppt": "promotionalPriceTier", "cmod": "priceModel" }, recommendation);
            recommendations.push(recommendation);
        }
        if (waNodes.length > 0) {
            _cacheManager.SetCacheItem(context.cacheKey, recommendations);
        }
        _invokeCallbackTag(context.callback, statusCode.Succeeded, recommendations, null, executor, 0x0080b75c);
    }
    ;
    return {
        initialize: function OSF_OmexProvider$initialize() {
            if (!_hasBeenInitialized) {
                _serviceEndPoint = Microsoft.Office.Common.XdmCommunicationManager.createServiceEndPoint(_clientID);
                _serviceEndPoint.registerMethod("OMEX_getAppDetailsAsync", this.getAppDetailsAsync, Microsoft.Office.Common.InvokeType.async, false);
                _serviceEndPoint.registerMethod("OMEX_getRecommendationsAsync", this.getRecommendationsAsync, Microsoft.Office.Common.InvokeType.async, false);
        ...
        getAppDetailsAsync: function OSF_OmexProvider$getAppDetailsAsync(params, callback) {
            ...
            try {
                ...
                var requestUrl = _urls.appDetailsWS + _queries.appDetailsWS;
                requestUrl = OSF.OUtil.formatString(requestUrl, params.contentMarket);
                ...
                params._onCompleteHandler = _onGetAppDetailsCompleted;
                _sendWebRequest(requestUrl, 'POST', { 'Content-Type': 'text/xml' }, _onCompleted, params, params.assetID);
            }
            catch (ex) {
                OsfMsAjaxFactory.msAjaxDebug.trace("Getting app details failed: " + ex);
                _invokeCallbackTag(callback, statusCode.Failed, null, "Getting app details failed.", null, 0x0085a2c8);
            }
        },
        getRecommendationsAsync: function OSF_OmexProvider$getRecommendationsAsync(params, callback) {
            OSF.OUtil.validateParamObject(params, {
                "applicationName": { type: String, mayBeNull: false },
                "officeExtentionTarget": { type: Number, mayBeNull: false },
                "billingMarket": { type: String, mayBeNull: false },
                "contentMarket": { type: String, mayBeNull: false },
                "clientName": { type: String, mayBeNull: true },
                "clientVersion": { type: String, mayBeNull: true }
            }, callback);
            var appVersion = _getAppVersion(params.applicationName);
            var appSubType = _getAppSubType(params.officeExtentionTarget);
            params.clearCache = params.clearCache || false;
            try {
                var cacheKey = OSF.OUtil.formatString(_recommendationsKey, params.billingMarket, params.contentMarket, appVersion, appSubType);
                if (params.clearCache) {
                    ...
                    if (value) {
                        _invokeCallbackTag(callback, statusCode.Succeeded, value, null, null, 0x0080b75e);
                        return;
                    }
                }
                params.callback = callback;
                params.cacheKey = cacheKey;
                var requestUrl = _urls.recommendationWS + _queries.recommendationWS;
                requestUrl = OSF.OUtil.formatString(requestUrl, params.billingMarket, params.contentMarket, appVersion, appSubType);
                var queryStringComponents = {};
                queryStringComponents[_queryStringParameters.clientName] = params.clientName;
                queryStringComponents[_queryStringParameters.clientVersion] = params.clientVersion;
                requestUrl += _createQueryStringFragment(queryStringComponents);
                params._onCompleteHandler = _onGetRecommendationsCompleted;
                _sendWebRequest(requestUrl, 'GET', { 'Content-Type': 'text/xml' }, _onCompleted, params);
            }
            catch (ex) {
                OsfMsAjaxFactory.msAjaxDebug.trace("Getting recommendations failed: " + ex);
                _invokeCallbackTag(callback, statusCode.Failed, null, "Getting recommendations failed.", null, 0x0085a2c9);
            }
        },
        clearCacheAsync: function OSF_OmexProvider$clearCacheAsync(params, callback) {
            var e = Function._validateParams(arguments, [{ name: "params", type: Object, mayBeNull: false },
            ...
            try {
                _cacheManager.RemoveAll(_cacheKeyPrefix);
                _invokeCallbackTag(callback, statusCode.Succeeded, null, null, null, 0x0080b760);
            }
            catch (ex) {
                OsfMsAjaxFactory.msAjaxDebug.trace("Clearing cache failed: " + ex);
                _invokeCallbackTag(callback, statusCode.Failed, null, "Clearing cache failed.", null, 0x0085a2ca);
            }
        }
    };
})();
OSF.OmexProvider.initialize();
