...
OSF.OmexGatedProvider = (function () {
    var _hasBeenInitialized = false;
    var _clientID = '78C04A0F-68A9-4C05-81E8-B39CC07A8CB2';
    var _cacheKeyPrefix = '__OSF_GATED_OMEX.';
    ...
    var _clientAppInstallInfoWS = {
        url: "/appinstall/authenticated?cmu={0}&cmf={1}&assetid={2}&ret={3}&rt=xml",
        cacheKey: _cacheKeyPrefix + "appinstall_authenticated.{0}.{1}.{2}.{3}"
    };
    var _killedAppsWS = {
    ...
    var _entitlementSummaryWS = {
        url: "/entitlement/query?av={0}&rt=xml",
        cacheKey: _cacheKeyPrefix + "entitlementSummary.{0}.{1}"
    };
    var _removeAppWS = {
        url: "/apps/remove?rt=xml"
    };
    ...
    function _onGetAppStateCompleted(executor) {
        ...
        _invokeCallbackTag(context.callback, statusCode.Succeeded, appState, null, executor, 0x0080b74b);
    }
    ;
    function _onGetEntitlementSummaryCompleted(executor) {
        ...
        entitlementsInfo.cid = context.cid;
        ...
        for (var i = 0; i < entitlementNodes.length; ++i) {
            ...
            xmlProcessor.readAttributes(entitlementNode, { "assetid": "assetId", "pid": "productId", "ver": "version", "appst": "appSubType", "cm": "contentMarket", "trlt": "licenseType", "acqdate": "acquiredDate", "expiry2": "expirationDate", "attreq": "attentionRequired" }, entitlement);
            entitlementsInfo.entitlements.push(entitlement);
        }
        if (entitlementNodes.length > 0) {
            _cacheManager.SetCacheItem(context.cacheKey, entitlementsInfo, entitlementsInfo.refreshRate / _hourToDayConversionFactor);
        }
        _invokeCallbackTag(context.callback, statusCode.Succeeded, entitlementsInfo, null, executor, 0x0080b74c);
    }
    ;
    function _onGetKilledAppsCompleted(executor) {
        ...
        _invokeCallbackTag(context.callback, statusCode.Succeeded, killedAppsInfo, null, executor, 0x0080b74d);
    }
    ;
    function _onGetClientAppInstallInfoCompleted(executor) {
        var context = executor.get_webRequest().get_userContext();
        var responseXml = executor.get_responseData();
        var xmlProcessor = new OSF.XmlProcessor(responseXml, _omexXmlNamespaces);
        var assetsNode = xmlProcessor.selectSingleNode("o:assets");
        var assetNode = xmlProcessor.selectSingleNode("o:asset", assetsNode);
        var manifestAndEToken = context.manifestAndEToken;
        ...
        if (manifestAndEToken.status === "1") {
            ...
            if (returnType != OSF.ClientAppInfoReturnType.etokenOnly) {
                context._onCompleteHandler = _onRetrieveManifestCompleted;
                _sendWebRequest(manifestAndEToken.url, 'GET', { 'Content-Type': 'text/xml' }, _onCompleted, context);
                return;
            }
        }
        _invokeCallbackTag(context.callback, statusCode.Succeeded, manifestAndEToken, null, executor, 0x0080b74e);
    }
    ;
    function _onRetrieveManifestCompleted(executor) {
        var context = executor.get_webRequest().get_userContext();
        var manifestAndEToken = context.manifestAndEToken;
        manifestAndEToken.manifest = executor.get_responseData();
        _invokeCallbackTag(context.callback, statusCode.Succeeded, manifestAndEToken, null, executor, 0x0080b74f);
    }
    ;
    function _onRemoveAppCompleted(executor) {
        var context = executor.get_webRequest().get_userContext();
        var responseXml = executor.get_responseData();
        var removedAppsInfo = {};
        ...
        for (var i = 0; i < appNodes.length; ++i) {
            ...
            if (removedApp.result == OSF.OmexRemoveAppStatus.Success) {
                cacheKeyPatterns.push(new RegExp(OSF.OUtil.formatString(_clientAppInstallInfoWS.cacheKey, REGEX_ANY_CHARACTERS, removedApp.assetId, REGEX_ANY_CHARACTERS, REGEX_ANY_CHARACTERS), "i"));
                hasSuccessResult = true;
            }
            removedAppsInfo.removedApps.push(removedApp);
        }
        if (hasSuccessResult) {
            cacheKeyPatterns.push(new RegExp(OSF.OUtil.formatString(_entitlementSummaryWS.cacheKey, REGEX_ANY_CHARACTERS, REGEX_ANY_CHARACTERS), "i"));
            _cacheManager.RemoveMatches(cacheKeyPatterns);
        }
        _invokeCallbackTag(context.callback, statusCode.Succeeded, removedAppsInfo, null, executor, 0x0080b750);
    }
    ;
    return {
        initialize: function OSF_OmexGatedProvider$initialize() {
            if (!_hasBeenInitialized) {
                _serviceEndPoint = Microsoft.Office.Common.XdmCommunicationManager.createServiceEndPoint(_clientID);
                _serviceEndPoint.registerMethod("OMEX_getEntitlementSummaryAsync", this.getEntitlementSummaryAsync, Microsoft.Office.Common.InvokeType.async, false);
                ...
                _serviceEndPoint.registerMethod("OMEX_removeAppAsync", this.removeAppAsync, Microsoft.Office.Common.InvokeType.async, false);
        ...
        getEntitlementSummaryAsync: function OSF_OmexGatedProvider$getEntitlementSummaryAsync(params, callback) {
            ...
            if (params.appVersion) {
                ...
                appVersion = _getAppVersion(params.applicationName);
            }
            params.clearEntitlement = params.clearEntitlement || false;
            params.cid = __omex_cid__ || 0;
            try {
                var cacheKey = OSF.OUtil.formatString(_entitlementSummaryWS.cacheKey, params.cid, appVersion);
                if (params.clearEntitlement) {
                    _cacheManager.RemoveCacheItem(cacheKey);
                }
                else {
                    var value = _cacheManager.GetCacheItem(cacheKey);
                    if (value) {
                        _invokeCallbackTag(callback, statusCode.Succeeded, value, null, null, 0x0080b751);
                        return;
                    }
                }
                params.callback = callback;
                params.cacheKey = cacheKey;
                var requestUrl = _entitlementSummaryWS.url;
                requestUrl = OSF.OUtil.formatString(requestUrl, appVersion);
                if (params.build) {
                    requestUrl = requestUrl + _buildParameter + params.build;
                }
                ...
                params._onCompleteHandler = _onGetEntitlementSummaryCompleted;
                _sendWebRequest(requestUrl, 'GET', { 'Content-Type': 'text/xml' }, _onCompleted, params);
            }
            catch (ex) {
                OsfMsAjaxFactory.msAjaxDebug.trace("Getting entitlements failed: " + ex);
                _invokeCallbackTag(callback, statusCode.Failed, null, "Getting entitlements failed.", null, 0x0085a29f);
            }
        },
        getKilledAppsAsync: function OSF_OmexGatedProvider$getKilledAppsAsync(params, callback) {
            var e = Function._validateParams(arguments, [{ name: "params", type: Object, mayBeNull: false },
            ...
            try {
                ...
                _invokeCallbackTag(callback, statusCode.Failed, null, "Getting killed bits failed.", null, 0x0085a2a0);
            }
        },
        getAppStateAsync: function OSF_OmexGatedProvider$getAppStateAsync(params, callback) {
            OSF.OUtil.validateParamObject(params, {
            ...
            try {
                ...
                _invokeCallbackTag(callback, statusCode.Failed, null, "Getting app state failed.", null, 0x0085a2a1);
            }
        },
        getManifestAndETokenAsync: function OSF_OmexGatedProvider$getManifestAndETokenAsync(params, callback) {
            OSF.OUtil.validateParamObject(params, {
                "assetID": { type: String, mayBeNull: false },
                "applicationName": { type: String, mayBeNull: false },
                "userContentMarket": { type: String, mayBeNull: false },
                "assetContentMarket": { type: String, mayBeNull: false },
                "clientName": { type: String, mayBeNull: true },
                "clientVersion": { type: String, mayBeNull: true }
            }, callback);
            ...
            params.cid = __omex_cid__ || 0;
            try {
                var cacheKey = OSF.OUtil.formatString(_clientAppInstallInfoWS.cacheKey, params.cid, params.assetID, params.userContentMarket, params.assetContentMarket);
                var value = { etoken: "", manifest: "", cached: false };
                var returnType = params.clientAppInfoReturnType ? params.clientAppInfoReturnType : OSF.ClientAppInfoReturnType.both;
                params.manifestAndEToken = value;
                params.returnType = returnType;
                params.callback = callback;
                params.cacheKey = cacheKey;
                var requestUrl = _clientAppInstallInfoWS.url;
                requestUrl = OSF.OUtil.formatString(requestUrl, params.userContentMarket, params.assetContentMarket, params.assetID, returnType);
                if (params.build) {
                    requestUrl = requestUrl + _buildParameter + params.build;
                }
                if (params.expectedVersion) {
                    requestUrl = requestUrl + _expectedVersionParameter + params.expectedVersion;
                }
                var queryStringComponents = {};
                queryStringComponents[_queryStringParameters.clientName] = params.clientName;
                queryStringComponents[_queryStringParameters.clientVersion] = params.clientVersion;
                requestUrl += _createQueryStringFragment(queryStringComponents);
                params._onCompleteHandler = _onGetClientAppInstallInfoCompleted;
                _sendWebRequest(requestUrl, 'GET', { 'Content-Type': 'text/xml' }, _onCompleted, params);
            }
            catch (ex) {
                OsfMsAjaxFactory.msAjaxDebug.trace("Getting client app installation info failed: " + ex);
                _invokeCallbackTag(callback, statusCode.Failed, null, "Getting client app installation info failed.", null, 0x0085a2a2);
            }
        },
        removeCacheAsync: function OSF_OmexGatedProvider$removeCacheAsync(params, callback) {
            OSF.OUtil.validateParamObject(params, {
                "assetID": { type: String, mayBeNull: false },
                "applicationName": { type: String, mayBeNull: false },
                "userContentMarket": { type: String, mayBeNull: false },
            ...
            try {
                ...
                if (params.clearEntitlement) {
                    ...
                    cacheKey = OSF.OUtil.formatString(_entitlementSummaryWS.cacheKey, cid, appVersion);
                    _cacheManager.RemoveCacheItem(cacheKey);
                }
                if (params.clearToken || params.clearManifest) {
                    cacheKey = OSF.OUtil.formatString(_clientAppInstallInfoWS.cacheKey, cid, params.assetID, params.userContentMarket, params.assetContentMarket);
                    var value = _cacheManager.GetCacheItem(cacheKey);
                    if (value) {
                        ...
                        _cacheManager.SetCacheItem(cacheKey, value, _manifestRefreshRate);
                    }
                }
                ...
                _invokeCallbackTag(callback, statusCode.Succeeded, null, null, null, 0x0080b756);
            }
            catch (ex) {
                OsfMsAjaxFactory.msAjaxDebug.trace("removing cache failed: " + ex);
                _invokeCallbackTag(callback, statusCode.Failed, null, "Removing cache failed.", null, 0x0085a2a3);
            }
        },
        clearCacheAsync: function OSF_OmexGatedProvider$clearCacheAsync(params, callback) {
            var e = Function._validateParams(arguments, [{ name: "params", type: Object, mayBeNull: false },
                { name: "callback", type: Function, mayBeNull: false }
            ]);
            if (e)
            ...
            try {
                ...
                _invokeCallbackTag(callback, statusCode.Succeeded, null, null, null, 0x0080b758);
            }
            catch (ex) {
                OsfMsAjaxFactory.msAjaxDebug.trace("Clearing cache failed: " + ex);
                _invokeCallbackTag(callback, statusCode.Failed, null, "Clearing cache failed.", null, 0x0085a2c0);
            }
        },
        removeAppAsync: function OSF_OmexGatedProvider$removeAppAsync(params, callback) {
            OSF.OUtil.validateParamObject(params, {
                "assetID": { type: String, mayBeNull: false },
                "clientName": { type: String, mayBeNull: true },
                "clientVersion": { type: String, mayBeNull: true }
            ...
            try {
                params.callback = callback;
                var requestUrl = _removeAppWS.url;
                var queryStringComponents = {};
                queryStringComponents[_queryStringParameters.clientName] = params.clientName;
                queryStringComponents[_queryStringParameters.clientVersion] = params.clientVersion;
                requestUrl += _createQueryStringFragment(queryStringComponents);
                params._onCompleteHandler = _onRemoveAppCompleted;
                _sendWebRequest(requestUrl, 'POST', { 'Content-Type': 'text/xml' }, _onCompleted, params, params.assetID);
            }
            catch (ex) {
                OsfMsAjaxFactory.msAjaxDebug.trace("Remove app failed: " + ex);
                _invokeCallbackTag(callback, statusCode.Failed, null, "Remove app failed.", null, 0x0085a2c1);
            }
        }
    };
})();
OSF.OmexGatedProvider.initialize();
