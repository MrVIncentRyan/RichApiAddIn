...
OfficeExt.appSpecificCheckOrigin = function (url, eventObj, messageObj, checkOriginFunction) {
    try {
        if (!eventObj || !eventObj.source || !eventObj.source.location || !eventObj.source.location.pathname || eventObj.source.location.pathname.indexOf("projection.aspx") < 0) {
            return false;
        }
        return checkOriginFunction(url, messageObj._origin);
    }
    catch (ex) {
        return false;
    }
};
//# sourceMappingURL=OsfRuntimeOutlookWeb.debug.js.map